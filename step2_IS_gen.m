...
for Benchmark=benchmark_list
    ...
    disp(['IS generation started with benchmark ',Benchmark])
    for yr=oos_range
        %% Generating IS data
        if ~exist(['dataset_',num2str(yr),'_IS_',num2str(IS_per),'_',Benchmark,'.mat'],'file')
            ...
            for s=-IS_per:1:-1
                ...
                curr_date_list=IS_db.date_friday;
                curr_prc_mat=IS_db.prc_mat;
                major_disruption=find(sum(isnan(curr_prc_mat),1)>=.8*size(curr_prc_mat,1));
                if numel(major_disruption)>0
                    disp(['Major disruption in stock market found on ',...
                        datestr(curr_date_list(major_disruption))])
                    curr_prc_mat(:,major_disruption)=curr_prc_mat(:,major_disruption-1);
                end
                for idx_asset=1:numel(stock_list_curr)
                    ...
                    asset_info{idx_asset,'full_name'}=curr_tbl1.conm(reg_ind_first);
            ...
            for s=1:numel(date_ser_IS)
                idx_bench_prc_IS=find(benchmark_data.datadate<=date_ser_IS(s),1,'last');
